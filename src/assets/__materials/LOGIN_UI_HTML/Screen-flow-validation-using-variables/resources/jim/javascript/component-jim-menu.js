    ...
    var menu = {
        ...
        "registerTimeout": function ($submenu) {
            if (jimUtil.exists($submenu)) {
                menu.defaults.timeoutRegister[$submenu.attr("id") + $submenu.attr("masteritem")] = setTimeout(function () {
                    menu.closeSubmenu($submenu);
                }, menu.defaults.hideDelay);
            }
        },
        ...
        "showMenu": function ($item) {
            if ($item.closest(".menu").length) {
                ...
                if ($item.closest(".menu").hasClass("horizontal")) {
                    ...
                    $submenu.css({
                        "z-index": Math.max($submenu.css("z-index"), menu.defaults.zIndex),
                        "top": position.bottom - 1 + "px",
