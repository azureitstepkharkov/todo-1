(function (window, undefined) {
    ...
    jQuery.fn.extend({
        "jimForceVisibility": function () {
            var $hidden = this.parentsUntil("#simulation").andSelf().filter(function () {
        ...
        "jimPosition": function (returnBBox) {
            var scroll = jimUtil.getScrollPosition(),
                simulationBounds = jQuery("#simulation")[0].getBoundingClientRect(), position, zoom = 1;
    ...
    var jimUtil = {
        ...
        "getCurrentScreen": function () {
            return jQuery.map(jQuery(".screen:visible"), function (canvas, i) {
                return canvas.id;
            })[0];
        },
        "getCanvases": function () {
            return jQuery.map(jQuery(".screen:visible, .template:visible, .master, .ui-scenario"), function (canvas, i) {
                return canvas.id;
            });
        },
        ...
        "getScrollbarWidth": function () {
            ...
            if (jimUtil.scrollbarWidth === 0) {
                if (jQuery.browser.msie) {
                    var $textarea1 = jQuery('<textarea cols="10" rows="2"></textarea>').css({
                            position: 'absolute',
                            top: -1000,
                            left: -1000
                        }).appendTo('body'),
                        $textarea2 = jQuery('<textarea cols="10" rows="2" style="overflow: hidden;"></textarea>').css({
                            position: 'absolute',
                            top: -1000,
                            left: -1000
                        }).appendTo('body');
            ...
            if (jimUtil.exists(args) && jimUtil.exists(args.target)) {
                ...
                switch (args.target.jimGetType()) {
                    case itemType.panel:
                        ...
                        args.target.siblings().addClass("hidden").css("display", "").end().css({
                            "z-index": "",
                            "position": "",
                            "top": "",
                            "left": ""
        ...
        "fitToScreen": function () {
            if (jimUtil.fitted && $(".ui-scenario").length == 0) {
                ...
                if (scale != 1 || jimUtil.getScale() != 1) {
                    ...
                    if (window.jimDevice.isMobile()) $("#zoomDiv #backgroundBox").not("#alignmentBox #backgroundBox").css({
                        "width": width / scale,
                        "height": height / scale
                    });
                    jQuery(".ui-page #zoomDiv").css({
        ...
        "jimPointTo": function ($target, settings) {
            if (jimUtil.exists($target) && $target.length && $target.is(":visible")) {
                var type = $target.jimGetType(), targetOffset, scrollParents, i, iLen, $current, current, offsetTop,
                    offsetLeft, dynamicPanel, scrollTop, scrollLeft, accumulatedScrollTop, accumulatedScrollLeft;
                if (jimUtil.isMobileDevice()) {
        ...
        "insertInto": function (args) {
            var $target, $parent, checkIntersect, positionIndex, position, event, $insert, $child, targetOffset,
                targetPosition, parentPosition, containment, point, positionType, displayType, x, y, index, insert,
                childOffset;
            $target = args.target;
            $parent = args.parent;
                    ...
                    if ($layout.length > 0 && $layout.hasClass("layout")) { /* container has non-free layout or rounded border */
                        ...
                        if ($insert.length) { /* container has non-free layout */
                            ...
                            if (jimUtil.intersect($target, $parent)) {
                                ...
                                $target.appendTo($parent).css({
                                    "position": "absolute",
                                    "top": targetPosition.top - parentPosition.top + $parent.scrollTop() - parseInt($parent.css("border-top-width"), 10),
                                    "left": targetPosition.left - parentPosition.left + $parent.scrollLeft() - parseInt($parent.css("border-left-width"), 10)
                                });
                            } else {
        ...
        "intersect": function (src, $over) {
            ...
            if (jimUtil.exists(src) && jimUtil.exists($over)) {
                if (src instanceof jQuery) {
                    ...
                    var scroll = jimUtil.getScrollPosition(),
                        simulationBounds = jQuery("#simulation")[0].getBoundingClientRect();
        ...
        "getRotatedTriangle": function (x, y, vertex, width, height, angleDegrees) {
            ...
            var bounds = [
                ...
                //upper left
                {
                    x: x + (halfWidth) * Math.cos(angle) - (halfHeight) * Math.sin(angle) + halfWidth,
                    y: y + (halfHeight) * Math.cos(angle) + (halfWidth) * Math.sin(angle) + halfHeight
                },
                //upper right
                {
                    x: x - (halfWidth) * Math.cos(angle) - (halfHeight) * Math.sin(angle) + halfWidth,
        ...
        "getPageMinSize": function ($canvas) {
            var $child, $children, $alignmentBox, i, iLen, minX, minY, maxX, maxY, defaultMasterWidth = 300,
                defaultMasterHeight = 300;
        ...
        "resizeCell": function ($cell, newCellWidth, newCellHeight, effect, updateTableFlag, callback) {
            var i, iLen, ownerIndex, $currentCell, $columnCells, $rowCells, $table, oldTableWidth, cellDeltaWidth,
                oldTableHeight, cellDeltaHeight;
        ...
        "resizeTable": function ($table, newTableWidth, newTableHeight, effect, callback) {
            var i, iLen, $cells, $currentCell, tableWidth, tableHeight, cellWidth, cellHeight,
                newTableWidthNoBorders = 0, newTableHeightNoBorders = 0;
        ...
        "wrapAllVerticalLayouts": function () {
            ...
            jQuery.each(cells, function (index, value) {
                wrapVerticalLayoutObject(value)
            });
        },
        ...
        "wrapAllHorizontalLayouts": function () {
            var cells = $(".horizontal:not(.verticalWrap)").filter(function (a, b) {
                return $(b).attr("hspacing") != undefined && ($(b).attr("hspacing") != 0 || $(b).attr("vspacing") != 0)
            });
            jQuery.each(cells, function (index, value) {
                wrapHorizontalLayoutObject(value, false)
            });
        },
        "wrapAllDataHorizontalLayouts": function () {
            var cells = $(".datalist .horizontal:not(.verticalWrap), .datagrid .horizontal:not(.verticalWrap)").filter(function (a, b) {
                return $(b).attr("hspacing") != undefined && ($(b).attr("hspacing") != 0 || $(b).attr("vspacing") != 0)
            });
            jQuery.each(cells, function (index, value) {
                wrapHorizontalLayoutObject(value, false)
    ...
    function wrapHorizontalLayoutObject(value, update) {
        ...
        if (width != undefined) {
            ...
            maxWidth = Math.max(maxWidth, /* tallest children */ (childs.size() > 0) ? childs.map(function (index, x) {
                return $(x).jimOuterWidth();
            }).toArray().reduce(function (sum, value) {
            ...
            if (wrap && vspacing != 0) jQuery.each(lastRow, function (i, v) {
                $(v).css("margin-bottom", 0);
            });
        }
    }
    ...
    function wrapVerticalLayoutObject(value, show) {
        ...
        obj.children(".verticalWrapper").each(function () {
        ...
        maxHeight = Math.max(maxHeight, /* tallest children */ childs.map(function (index, x) {
            return $(x).jimOuterHeight();
        }).toArray().reduce(function (sum, value) {
            return Math.max(value, sum);
        }));
