...
(function (window, undefined) {
    ...
    var jimResponsive = {
        ...
        "refreshResponsiveComponents": function ($component, onlySynced, sizeChange, updatePercentage, canvasLoad) {
            ...
            var cells = $target.find(".horizontal.wrap:not(.verticalWrap)").filter(function (a, b) {
                return $(b).attr("hspacing") != undefined && ($(b).attr("hspacing") != 0 || $(b).attr("vspacing") != 0)
            });
            jQuery.each(cells, function (index, value) {
                jimUtil.wrapHorizontalLayout(value);
            });
            jQuery.each($target.find(".verticalWrap"), function (index, value) {
                jimUtil.wrapVerticalLayout(value);
            });
