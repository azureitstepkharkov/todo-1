    ...
    jQuery.extend(jimEvent.fn, {
        ...
        "jimMove": function (args, callback) {
            if (jimUtil.exists(args)) {
                var self = this, $targets, t, tLen, $target, eventData, dragData, position, newTopPosition,
                    newLeftPosition, $parent, containment, $svg, effect, top, left, topPinOffset, leftPinOffset;
                var CCWrapper = true;
                var properties = {};
        ...
        "jimInsert": function (args, callback) {
            if (jimUtil.exists(args) && jimUtil.exists(args.target)) {
                ...
                for (t = 0, tLen = $targets.length; t < tLen; t += 1) {
                    ...
                    var vWrap = $target.closest(".layout.horizontal");
                    if (vWrap.length > 0) {
                        if (dragData && vWrap.hasClass("verticalWrap")) $target.css({
                            "position": "",
                            "top": "",
                            "left": "",
                            "display": ""
                            ...
                            for (i = 0, iLen = $targets.length; i < iLen; i += 1) {
                                ...
                                switch (type) {
                                    ...
                                    case itemType.nativedropdown:
                                        ...
                                        self.jimSetSelection({
                                            "target": $target,
                                            "value": $target.children(".dropdown-options").children(".option:first").text()
                                        });
                                        break;
                                    case itemType.selectionlist:
                                    case itemType.multiselectionlist:
                                ...
                                if (bShape && $target.hasClass("autofit")) {
                                    ...
                                    shapeStyle.attributes = {
                                        ...
                                        "height": "auto",
                                        "text-height": parseInt($target.find(".content").css('height'))
                                    };
        ...
        "jimSetSelection": function (args, instance, callback) {
            if (jimUtil.exists(args) && jimUtil.exists(args.value) && jimUtil.exists(args.target)) {
                var self = this, $targets, $target, type, value = "", $options, $option, option, t, tLen, o, oLen, v,
                    vLen;
        ...
        "jimResize": function (args, callback) {
            if (jimUtil.exists(args)) {
                var self = this, $targets, $parent, type, i, iLen, width, height, bShape, shapeStyle, percentageWidth,
                    percentageHeight, parentWidth, parentHeight;
                if (args.target) {
...
//                  	height = (height * 100 / parentHeight) +"%";
                                ...
                                if (args.effect) {
                                    if (bShape) {
                                        var shapeProgress = function () {
                                            ...
                                            shapeStyle = {};
                                            shapeStyle.attributes = {
                                                "width": $shapewrapper.css("width"),
                                                "height": $shapewrapper.css("height")
                                            };
                            ...
                            if (!args.effect || $target.is(".gridcell")) jQuery.each(vWrap, function (index, value) {
                                jimUtil.wrapLayout(value);
                            });
