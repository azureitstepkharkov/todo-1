        ...
        jQuery(".firer")
            ...
            .bind("input", function (event, data) { // Almost every key in the soft keyboard trigger this function
                ...
                if (!(data && data.preventTrigger)) { /* prevent infinite loop */
                    $firer.trigger("keyup.jim", [{
                        "preventTrigger": true,
                        "altKey": event.altKey ? event.altKey : false,
                        "ctrlKey": event.ctrlKey ? event.ctrlKey : false,
                        "shiftKey": event.shiftKey ? event.shiftKey : false,
                        "which": which
                    }]);
                }
            })
        ...
        .bind("touchstart mousedown gesturestart gestureend touchend mouseup touchmove", function (event) {
            ...
            switch (event.type) {
                ...
                case "gesturestart":
                    ...
                    jFirer.data("jimGesture", {
                        "startX": (hasTouches) ? touches[0].pageX : event.pageX,
                        "startY": (hasTouches) ? touches[0].pageY : event.pageY
                    });
                    break;
                case "gestureend":
                ...
                case "touchmove":
                    if (jimUtil.isAndroidDevice() && hasTouches && touches.length == 2) { //pinch-rotate cases in android
                        if (!jFirer.data("jimStartGesture"))
                            jFirer.data("jimStartGesture", {
                                "startX": [touches[0].pageX, touches[1].pageX],
                                "startY": [touches[0].pageY, touches[1].pageY]
                            });
                        else
                            jFirer.data("jimEndGesture", {
                                "endX": [touches[0].pageX, touches[1].pageX],
                                "endY": [touches[0].pageY, touches[1].pageY]
                            });
                    }
                    break;
            }
