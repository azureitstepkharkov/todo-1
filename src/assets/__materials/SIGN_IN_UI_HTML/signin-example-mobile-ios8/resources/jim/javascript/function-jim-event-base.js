...
(function (window, undefined) {
    jQuery.extend(jimEvent.fn, {
            ...
            else
            {
                ...
                if (newCases.length === 1) {
                    ...
                    for (i = 0, bLen = newCases.length; i < bLen; i += 1) {
                        ...
                        (function (caze) {
                            jimEvent.pauseStack.push(setTimeout(function () {
                                self.executeBlocks(caze.blocks, caseCallback)
                            }, maxDelay));
                        })(current);
                    }
                }
        ...
        "undoCases": function ($firer) {
            var self = this, undoPauseStack, undoStack, newUndoStack, $eventFirer, undoAction, undoType, c, cLen,
                doRestore = true;
            newUndoStack = [];
        ...
        "executeActions": function (actions, caseCallback) {
            if (jimUtil.exists(actions)) {
                //preprocess actions
                var self = this, a, aLen, b, bLen, $firer, undo, undoStack, lastUndo, lastUndoAction, current,
                    currentTarget, undoTarget, maxDelay;
                ...
                for (b = 0, bLen = len; b < bLen; b += 1) {
                    ...
                    if (current.action === "jimNavigation") {
                        if (current.exectype === 'serial' && current.delay === 0)
                            ...
                            (function (action) {
                                jimEvent.pauseStack.push(setTimeout(function () {
                                    jimEvent.fn[action.action].call(self, action.parameter)
                                }, maxDelay));
                            })(current);
                        }
                        break;
                        ...
                        if (current.exectype === 'serial' && current.delay === 0)
                            ...
                            (function (action) {
                                jimEvent.pauseStack.push(setTimeout(function () {
                                    jimEvent.fn[action.action].call(self, action.parameter, undefined, actionCallback)
                                }, maxDelay));
                            })(current);
                        }
                    } else {
                        if (current.exectype === 'serial' && current.delay === 0)
                            ...
                            (function (action) {
                                jimEvent.pauseStack.push(setTimeout(function () {
                                    jimEvent.fn[action.action].call(self, action.parameter, actionCallback)
                                }, maxDelay));
                            })(current);
                        }
                    }
...
//                $target =$("[id="+$wrapper.prop('id')+"]");
        ...
        "getUndoActions": function (action) {
            var self = this, actionType = action.action, undoActions = [], undoAction, $targets, $target, target, type,
                len, isReadonly, style, s, undoStyle, property, t, tLen, bShape;
            switch (actionType) {
                case "jimChangeStyle":
                    undoAction = {
                            ...
                            case "jimSetValue":
                                ...
                                switch (type) {
                                    ...
                                    case itemType.checkboxlist:
                                        undoActions.push(self.getUndoActions({
                                            "action": "jimSetSelection",
                                            "parameter": {"target": action.parameter.target}
                                        })[0]);
                                        break;
                                    case itemType.image:
